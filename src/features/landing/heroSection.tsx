...
class Node {
  ...
  isHovered(): boolean {
    return this.p5.dist(this.position.x, this.position.y, this.p5.mouseX, this.p5.mouseY) < 20;
  }
...
const HeroSection: React.FC = () => {
  ...
  const sketch: Sketch = (p5: p5) => {
    ...
    p5.setup = () => {
      ...
      rootNode = new Node(p5, 0, "", 600*scalefactor, p5.width / 2, p5.height-50);
      ...
      let Exploration = new Node(p5, 4, "Exploration", 50, p5.random(0, p5.width), p5.random(0, p5.height), ROUTES.EXPLORATION, router);
      ...
      let Kaleidoscope = new Node(p5, 4, "Kaleidoscope", 50, p5.random(0, p5.width), p5.random(0, p5.height), ROUTES.KALEIDOSCOPE, router);
      let Tabulature = new Node(p5, 4, "Tabulature", 50, p5.random(0, p5.width), p5.random(0, p5.height), ROUTES.TABULATURE, router);
    ...
    p5.windowResized = () => {
      ...
      if(p5.abs(width - p5.windowWidth)/width > .2){
        p5.setup();
        width = p5.windowWidth;
      }
      if(p5.abs(height - p5.windowHeight)/height > .2){
        p5.setup();
